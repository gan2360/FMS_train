...
parser.add_argument('--dataset_dir', type=str, default='./PIMDataset_faster/PIMDataset_faster/', help='Experiment path')
...
parser.add_argument('--epoch', type=int, default=10, help='The time steps you want to subsample the dataset to,500') # 默认500
parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate')
parser.add_argument('--batch_size', type=int, default=4, help='Batch size,128')
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs + 1, args.epoch):
        ...
        for i_batch, sample_batched in bar(enumerate(train_dataloader, 0)):
            ...
            # heatmap = torch.tensor(sample_batched[2], dtype=torch.float, device=device) # 第二版不需要
            ...
            if i_batch % 185 == 0 and i_batch != 0:
                print("[%d/%d] LR: %.6f, Loss: %.6f, Keypoint_loss: %.6f, "
                      "k_max_gt: %.6f, k_max_pred: %.6f, k_min_gt: %.6f, k_min_pred: %.6f, " % (
                          i_batch, len(train_dataloader), get_lr(optimizer), loss.item(), loss_keypoint,
            ...
            wandb.log({'loss/train_loss': avg_train_loss}, step=train_record_count)
